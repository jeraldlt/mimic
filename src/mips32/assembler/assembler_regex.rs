...
fn assemble(lines: Vec<String>) -> Result<(Vec<u8>, Vec<u8>), MimicError> {
    ...
    for mut line in lines {
        ...
        match cur_vec {
            CurVec::None => {}
            CurVec::Text => text.push(line.clone()),
            CurVec::Data => data.push(line.clone()),
        }

        match line.to_lowercase().as_str() {
            ".data" => cur_vec = CurVec::Data,
            ".text" => cur_vec = CurVec::Text,
            _ => {}
        }
    }
...
fn register_name_to_number(reg: String) -> Result<usize, MimicError> {
